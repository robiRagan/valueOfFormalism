...
#' 
#' This file is a general version of the code that will allow a user to change the parameters of the simulation and run versions not found in the paper.
#' There are accompaning R files that allow for "push-button" replication of each of the figures in the paper.
#' 
#'    replicationFigure4A.R
#'    replicationFigure4B.R
#'    replicationFigure4C.R 
#'    replicationFigure4D.R 
#'    replicationFigure6A.R 
#'    replicationFigure6B.R 
#'    replicationFigure7A.R
#'    replicationFigure7B.R 
#'    replicationFigure8A.R 
#'    replicationFigure8B.R 
#'     
...
#'  To use this file:
#'  
#'  1. Run all of the code from line 54 to 997
#'  2. The code starting at line 1000 is where you can make changes to run your own version of the simulation.
#'  3. Once you have changes the parameters to your liking run lines 1004 t0 1024 to run the simulation.
#'  4. Be aware that certian parameter combinations will take quite a while to run.
#'  
#'       
#' ---------------------------------------
...
#' ---------------------------------------
#' Groups: 
#'      Voters are assigned to a voter group. The number of voters in each group and the group level mean, and standard deviation for both utility and error are explicitly set by the user. 
#' 
#' Utility and Errors: 
#'      Drawn from a Normal(groupMean, groupStandardDeviation). Where groupMean and groupStandardDeviation are subgroup specific.
#'
#'  Voting on multiple proposals in a series:
#'      Voting takes place using a "successive procedure", the initial status quo q1 is paired against a proposal x1 in round 1. If x1 passes, voting ends. If x1 fails, q1 is paired against x2 in round 2, and so on for a total of R rounds (see Figure 5 in the paper.). In a standard successive procedure, voting continues until the proposal passes or a specific round R is reached and the proposal either passes or fails. In our study, voting continues until the proposal eventually passes, or a predefined maximum number of rounds, maximumNumberOfProposalsInASeries, is reached.
#'      - Voters vote yea for a proposal if ui + ei > 0
#'      - Voters individually incur an external cost if they are harmed when a proposal passes. 
#'          - This individual loss is eci = |ui| if ui < 0 and a proposal passes.
#'      - External Cost for a round of voting is the sum of all the individual external costs, in the round a proposal passes. 
#'      - Decision Cost is c*r. Where c is per-round decision cost and r is the number of rounds that have ellapesed.
#'      - Total Cost is External Cost + Decision Cost
#'                  
#'  Iterations:
#'    Each iteration begins with the same initial set of parameters, and runs multiple proposals in a series of voters, for J iterations. Averaging the External Costs, Decision Costs, and Total Costs found in each iteration, approximates the expected values of the respective costs. It is those averages that are the model output presented in the paper. 
#'  
#' The Simulations:  
#'  The aKMajorityCostMultiSimulation() function runs an entire simulation. This means it runs multiple iterations, stores the input parameters, model outup and plots the output.
#'  
#############################################################################################################################
#'   ----------------------------------------------------------------------
...
##################################################################
#   Run all lines to here before running the simulation function
###################################################################


aSimulation <- aKMajorityCostMultiSimulation(folderName="test", 
                                            numberOfIterations=1000, # How many iterations of the same model parameters do you want to run?
                                            groupSize=c(33,33,34),   # The number of voters in each of the three groups.
                                            utilityDistribution=c("normal","normal","normal"), #Leave this one, only normal is implemented in this code. 
                                            utilityDistributionParameters=c(-.25,.25,0,.25,.25,.25), # (Mean1,Std1, Mean2, Std2, Mean3, Std3) for utility.
                                            errorDistribution=c("normal","normal","normal"), #Leave this one, only normal is implemented in this code.
                                            errorDistributionParameters=c(0,.3,0,.3,0,.3), # (Mean1,Std1, Mean2, Std2, Mean3, Std3) for error.
                                            groupPostFailingProposalMeanUiIncrease=c(".01",".01",".01"), # This is the alpha parameter in the paper.
                                            maximumNumberOfProposalsInASeries=10000, # Some sims never pass a proposal (see Figure8A) so set this to 100000 to be sure it ends. Can also be set FALSE
                                            perProposalDecisionCost=.01,# This is parameter c in the paper.
                                            outputTo=getwd(), # This uses your current folder as the working directory. Change only if you know what you're doing :)
                                            silentSeries=FALSE, # prints where you are in the sim to the screen
                                            silentIterations=FALSE, # prints where you are in the sim to the screen
                                            writeCSV=FALSE, # set to TRUE to write out the output as CSV
                                            writeRObjects=FALSE, # set to TRUE to write out the output as an RObject
                                            plotExternalDecisionTotalCosts=TRUE, # set to TRUE for multi round runs.
                                            plotOnlyExternalCost=FALSE,  # set to FALSE for multi round runs.
                                            plotNumberOfRounds=FALSE, # A diagnostic plot you can turn on or off.
                                            plotPareto=TRUE, # Like bottom panel in Figure 8B.
                                            printOutputToScreen=TRUE) #Prints a summary to the screen when sim is over. 
